...
if($newstatus==0){
    ...
    $updateQuery = "UPDATE mymovielist SET status = '".$newstatus."', myscore = '$newScore', movie_score = '$newTotalScore' WHERE movie_id = ".$_POST['movie_id']." AND accounts_id = " . $_SESSION['accounts_id'];
    $updateQuery3 = "UPDATE mymovielist SET movie_score = '$newTotalScore'  WHERE movie_id = '$movieId'";

    // Güncelleme sorgusunu çalıştırın
    if (mysqli_query($connection, $updateQuery) && mysqli_query($connection, $updateQuery1) && mysqli_query($connection, $updateQuery3)) {
